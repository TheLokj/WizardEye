...
def generate_standard_database(
    ...
    for query_fasta in query_fastas:
        ...
        cmd = [
            ...
            "-k", str(kmer_length),
            "-w", str(offset_step),
            "-bn", str(bwa_missing_prob_err_rate),
            "-bo", str(bwa_max_gap_opens),
            "-bl", str(bwa_seed_length),
            "-j", str(n_threads),
            "-cs", str(chunk_size),
            "-d", str(db_path),
        ]
...
def test_align_same_behavior_as_original_script():
    ...
    for query_fa in query_fastas:
        ...
        with tempfile.TemporaryDirectory(prefix="wizardeye_test_original_") as original_tmpdir, \
             tempfile.TemporaryDirectory(prefix="wizardeye_test_wizardeye_") as wizardeye_tmpdir:
            ...
            print(f"Original script return code for {query_fa.name}: {result.returncode}")
            if result.returncode != 0:
                raise RuntimeError(f"Original script execution failed with return code {result.returncode}")
            
            # Verify original script created expected outputs in output/tracks/
            tracks_dir = original_output / "tracks"
            assert tracks_dir.exists(), f"Original script tracks directory not found: {tracks_dir}"
            ...
            assert original_cov_map_bw.exists(), f"Original script did not create {original_cov_map_bw}"
            assert original_cov_uniq_bw.exists(), f"Original script did not create {original_cov_uniq_bw}"
        
            # Initialize WizardEye database
            subprocess.run(
                ["python3", "-m", "wizardeye", "database", "--init", "-d", str(wizardeye_tmpdir)],
                check=True,
                capture_output=True,
                text=True,
                env={**subprocess.os.environ, "PYTHONPATH": str(SRC_DIR)}
            ...
            with tempfile.TemporaryDirectory(prefix="wizardeye_compare_") as compare_dir:
                ...
                subprocess.run(["bigWigToBedGraph", str(original_cov_map_bw), str(orig_map_all_bg)], check=True)
                subprocess.run(["bigWigToBedGraph", str(original_cov_uniq_bw), str(orig_map_uniq_bg)], check=True)
                subprocess.run(["bigWigToBedGraph", str(wizardeye_map_all_bw), str(we_map_all_bg)], check=True)
                subprocess.run(["bigWigToBedGraph", str(wizardeye_map_uniq_bw), str(we_map_uniq_bg)], check=True)
                
                # Compare: cov_map_* (original) vs map_all (WizardEye) - both represent all mappings
                assert compare_bedgraph_files(orig_map_all_bg, we_map_all_bg, f"map_all ({query_fa.name})"), \
                ...
                assert compare_bedgraph_files(orig_map_uniq_bg, we_map_uniq_bg, f"map_uniq ({query_fa.name})"), \
...
def test_export_same_behavior_as_original_script():
    ...
    with tempfile.TemporaryDirectory(prefix="wizardeye_export_original_") as original_tmpdir, \
         tempfile.TemporaryDirectory(prefix="wizardeye_export_wizardeye_") as wizardeye_tmpdir:
        ...
        original_mask = original_output / f"all_overlaps_mask_{bn_str}_k{STANDARD_KMER_LENGTH}.bed"
        assert original_mask.exists(), f"Original script did not create export mask: {original_mask}"
        ...
        wizardeye_export_cmd = [
            "python3", "-m", "wizardeye", "export",
            "-r", str(hg19_stem),
            "--exclude-tracks", exclude_tracks_str,
            "-k", str(STANDARD_KMER_LENGTH),
            "-w", str(STANDARD_OFFSET_STEP),
            "-bn", str(STANDARD_BWA_MISSING_PROB_ERR_RATE),
            "-bo", str(STANDARD_BWA_MAX_GAP_OPENINGS),
            "-bl", str(STANDARD_BWA_SEED_LENGTH),
            "-p", str(STANDARD_CROSS_STRINGENCY),
            "-o", str(wizardeye_mask),
            "-d", str(wizardeye_db),
        ]
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"wizardeye export execution failed with return code {result.returncode}")
        
        assert wizardeye_mask.exists(), f"wizardeye export did not create mask: {wizardeye_mask}"
        ...
        wizardeye_mask_merged = wizardeye_db / "export" / "wizardeye_mask_all_species_merged.bed"
        subprocess.run(
        ...
        with tempfile.TemporaryDirectory(prefix="wizardeye_export_compare_") as compare_dir:
            compare_path = Path(compare_dir)
            ...
            subprocess.run(["sort", str(original_mask), "-o", str(orig_sorted)], check=True)
            subprocess.run(["sort", str(wizardeye_mask_merged), "-o", str(we_sorted)], check=True)
            ...
            if len(orig_lines) != len(we_lines):
                print(f"Different number of lines: original={len(orig_lines)}, wizardeye={len(we_lines)}")
...
def test_consistency_align_same_launch():
    ...
    for query_fa in query_fastas:
        ...
        with tempfile.TemporaryDirectory(prefix=f"wizardeye_consistency_align_{query_fa.stem}_") as base_tmpdir:
            ...
            for rep in range(n_repetitions):
                with tempfile.TemporaryDirectory(prefix=f"rep{rep}_") as tmpdir:
                    ...
                    if result.returncode != 0:
                        print(f"wizardeye align stderr for {query_fa.name} rep {rep}: {result.stderr}")
                        print(f"wizardeye align stdout for {query_fa.name} rep {rep}: {result.stdout}")
                        raise RuntimeError(f"wizardeye align execution failed with return code {result.returncode}")
            ...
            for rep in range(1, n_repetitions):
                ...
                with tempfile.TemporaryDirectory(prefix=f"wizardeye_compare_consistency_{query_fa.stem}_rep{rep}_") as compare_dir:
                    compare_path = Path(compare_dir)
                    ...
                    # Compare map_all
                    assert compare_bedgraph_files(first_all_bg, current_all_bg, f"map_all rep0 vs rep{rep} ({query_fa.name})"), \
                        f"map_all.bw differs between rep 0 and rep {rep} for {query_fa.name}"
                    
                    # Compare map_uniq
                    assert compare_bedgraph_files(first_uniq_bg, current_uniq_bg, f"map_uniq rep0 vs rep{rep} ({query_fa.name})"), \
...
def test_consistency_align_parallelisation():
    ...
    for query_fa in query_fastas:
        ...
        with tempfile.TemporaryDirectory(prefix=f"wizardeye_consistency_parallel_{query_fa.stem}_") as base_tmpdir:
            base_path = Path(base_tmpdir)
            ...
            for n_threads in thread_counts:
                with tempfile.TemporaryDirectory(prefix=f"threads{n_threads}_") as tmpdir:
                    wizardeye_db = Path(tmpdir) / "database"
                    wizardeye_db.mkdir(parents=True)
                    ...
                    if result.returncode != 0:
                        print(f"wizardeye align stderr for {query_fa.name} threads {n_threads}: {result.stderr}")
                        print(f"wizardeye align stdout for {query_fa.name} threads {n_threads}: {result.stdout}")
                        raise RuntimeError(f"wizardeye align execution failed with return code {result.returncode}")
                    ...
                    bw_files[n_threads] = (dest_dir / "map_all.bw", dest_dir / "map_uniq.bw")
            ...
            for n_threads in thread_counts[1:]:
                ...
                with tempfile.TemporaryDirectory(prefix=f"wizardeye_compare_parallel_threads{n_threads}_") as compare_dir:
                    compare_path = Path(compare_dir)
                    ...
                    # Compare map_all
                    assert compare_bedgraph_files(first_all_bg, current_all_bg, f"map_all threads{first_threads} vs threads{n_threads} ({query_fa.name})"), \
...
def test_consistency_filter_parallelisation():
    ...
    # Use a single persistent temp directory for all thread counts
    with tempfile.TemporaryDirectory(prefix="wizardeye_consistency_filter_parallel_") as base_tmpdir:
        base_path = Path(base_tmpdir)
        ...
        for n_threads in thread_counts:
            with tempfile.TemporaryDirectory(prefix=f"filter_threads{n_threads}_") as tmpdir:
                ...
                wizardeye_cmd = [
                    "python3", "-m", "wizardeye", "filter",
                    "-i", str(SIMULATED_URSUS_BAM),
                    "-r", str(hg19_stem),
                    "-k", str(STANDARD_KMER_LENGTH),
                    "-w", str(STANDARD_OFFSET_STEP),
                    "-bn", str(STANDARD_BWA_MISSING_PROB_ERR_RATE),
                ...
                if result.returncode != 0:
                    print(f"wizardeye filter stderr threads {n_threads}: {result.stderr}")
                    print(f"wizardeye filter stdout threads {n_threads}: {result.stdout}")
                    raise RuntimeError(f"wizardeye filter execution failed with return code {result.returncode}")
                
                assert report_path.exists(), f"wizardeye filter did not create report: {report_path}"
...
def test_consistency_count_same_launch():
    ...
    # Use a single persistent temp directory for all repetitions
    with tempfile.TemporaryDirectory(prefix="wizardeye_consistency_count_same_launch_") as base_tmpdir:
        base_path = Path(base_tmpdir)
        ...
        for rep in range(n_repetitions):
            with tempfile.TemporaryDirectory(prefix=f"count_rep{rep}_") as tmpdir:
                ...
                if result.returncode != 0:
                    ...
                    raise RuntimeError(f"wizardeye count execution failed with return code {result.returncode}")
                
                assert report_path.exists(), f"wizardeye count did not create report: {report_path}"
...
def test_consistency_count_parallelisation():
    ...
    # Use a single persistent temp directory for all thread counts
    with tempfile.TemporaryDirectory(prefix="wizardeye_consistency_count_parallel_") as base_tmpdir:
        base_path = Path(base_tmpdir)
        ...
        for n_threads in thread_counts:
            with tempfile.TemporaryDirectory(prefix=f"count_threads{n_threads}_") as tmpdir:
                output_dir = Path(tmpdir) / "output"
                output_dir.mkdir(parents=True)
                ...
                if result.returncode != 0:
                    print(f"wizardeye count stderr threads {n_threads}: {result.stderr}")
                    print(f"wizardeye count stdout threads {n_threads}: {result.stdout}")
                    raise RuntimeError(f"wizardeye count execution failed with return code {result.returncode}")
...
def test_consistency_filter_same_launch():
    ...
    # Use a single persistent temp directory for all repetitions
    with tempfile.TemporaryDirectory(prefix="wizardeye_consistency_filter_same_launch_") as base_tmpdir:
        base_path = Path(base_tmpdir)
...
def test_export_and_filter_same_results():
    ...
    for cross_stringency in stringency_values:
        with tempfile.TemporaryDirectory(prefix=f"wizardeye_export_filter_compare_s{cross_stringency}_") as base_tmpdir:
            base_path = Path(base_tmpdir)
            ...
            filter_tmpdir = tempfile.TemporaryDirectory(prefix="filter_", dir=base_tmpdir)
            ...
            filter_cmd = [
                ...
                "-bo", str(STANDARD_BWA_MAX_GAP_OPENINGS),
                "-bl", str(STANDARD_BWA_SEED_LENGTH),
                "-p", str(cross_stringency),
                "-j", str(STANDARD_N_THREADS),
                "--exclude-tracks", exclude_tracks_str,
                "--report-output", str(filter_report_path),
                "--excluded-output", str(filter_excluded_bam),
                "--export-bam",
                "-d", str(wizardeye_db),
            ]
            ...
            assert filter_report_path.exists(), f"wizardeye filter did not create report: {filter_report_path}"
            assert filter_excluded_bam.exists(), f"wizardeye filter did not create excluded BAM: {filter_excluded_bam}"
            
            # Run WizardEye export to get BED mask
            export_tmpdir = tempfile.TemporaryDirectory(prefix="export_", dir=base_tmpdir)
            ...
            assert export_mask_path.exists(), f"wizardeye export did not create mask: {export_mask_path}"
            


            # Use bedtools intersect to find reads overlapping the export mask
            intersect_tmpdir = tempfile.TemporaryDirectory(prefix="intersect_", dir=base_tmpdir)
            ...
            subprocess.run(
                ["samtools", "sort", str(SIMULATED_URSUS_BAM), "-o", str(sorted_input_bam)],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            result = subprocess.run(
                [
                    "bedtools", "intersect",
                    "-abam", str(sorted_input_bam),
                    "-b", str(export_mask_path),
                    "-wa"
                ],
            ...
            subprocess.run(
                ["samtools", "sort", str(intersect_bam_path), "-o", str(intersect_sorted_bam_path)],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            subprocess.run(
                ["samtools", "sort", str(filter_excluded_bam), "-o", str(filter_excluded_sorted)],
                check=True,
                capture_output=True,
                text=True,
            )
